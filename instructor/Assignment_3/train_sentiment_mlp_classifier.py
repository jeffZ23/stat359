...
def mean_pool_fasttext(text: str, ft: KeyedVectors, dim: int = 300) -> np.ndarray:
    toks = tokenize(text)
    vecs = []
    for t in toks:
        if t in ft:
...
def precompute_embeddings(texts: List[str], ft: KeyedVectors, dim: int = 300) -> np.ndarray:
    embs = np.zeros((len(texts), dim), dtype=np.float32)
    for i, t in enumerate(texts):
        embs[i] = mean_pool_fasttext(t, ft, dim=dim)
    return embs
...
class MLP(nn.Module):
    def __init__(self, in_dim: int, hidden_dims: List[int], num_classes: int, dropout: float):
        super().__init__()
...
class TrainConfig:
    ...
    out_dir: str = "artifacts/mlp_fasttext"
    best_path: str = "artifacts/mlp_fasttext/best_mlp_fasttext.pt"
...
def main():
    ...
    tr_emb = precompute_embeddings(x_tr, ft, dim=dim)
    va_emb = precompute_embeddings(x_va, ft, dim=dim)
    te_emb = precompute_embeddings(x_te, ft, dim=dim)

    train_ds = EmbeddingDataset(tr_emb, y_tr)
    val_ds = EmbeddingDataset(va_emb, y_va)
    test_ds = EmbeddingDataset(te_emb, y_te)
    ...
    model = MLP(in_dim=dim, hidden_dims=list(cfg.hidden_dims), num_classes=num_classes, dropout=cfg.dropout).to(device)
...
# {"best_epoch": 25, "best_val_macro_f1": 0.7202215674334713, "test_loss": 0.9161671799347522, "test_acc": 0.7537826685006878, "test_macro_f1": 0.7115113716488661}